...
class SpecAugment(nn.Module):
    ...
    def _sample_axis_mask(
        ...
        *,
        ...
        if num_masks <= 0 or axis_length <= 0 or max_width <= 0:
            return torch.zeros(batch_size, axis_length, device=device, dtype=torch.bool)

        max_width = min(int(max_width), int(axis_length))
        ...
        max_starts = axis_length - widths + 1
        starts = torch.floor(torch.rand((batch_size, num_masks), device=device) * max_starts.to(torch.float32)).to(
            torch.long
        )

        positions = torch.arange(axis_length, device=device).view(1, 1, axis_length)
        mask = (positions >= starts.unsqueeze(-1)) & (positions < (starts + widths).unsqueeze(-1))
    ...
    def forward(self, spec: torch.Tensor) -> Tuple[torch.Tensor, Dict[str, torch.Tensor]]:
        ...
        freq_mask = self._sample_axis_mask(
            batch_size,
            num_mels,
            self.num_freq_masks,
        ...
        time_mask = self._sample_axis_mask(
            batch_size,
            num_frames,
            self.num_time_masks,
        ...
        aug_spec = spec.masked_fill(freq_mask[:, None, :, None], 0)
        aug_spec = aug_spec.masked_fill(time_mask[:, None, None, :], 0)

        apply_augment = torch.rand((), device=device) <= self.p
        aug_spec = torch.where(apply_augment, aug_spec, spec)
        freq_mask = freq_mask & apply_augment
        time_mask = time_mask & apply_augment
